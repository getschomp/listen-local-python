...
import listen_local.integrations.spotify_oauth as module
...
def test_get_basic_auth_header():
    assert module._get_basic_auth_header() == {'Authorization': 'Basic Y2xpZW50X2lkOmNsaWVudF9zZWNyZXQ='}


@mock.patch.object(module, 'url_for', return_value='/redirect')
def test_get_code_payload(code):
    module._get_code_payload('code')
    assert  {
